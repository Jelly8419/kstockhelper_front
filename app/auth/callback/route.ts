import { NextResponse } from "next/server";
import { createClient } from "@/lib/supabase/server";
import { PRIVACY_POLICY, TERMS_OF_SERVICE } from "@/lib/legal/content";
import { routing } from "@/lib/i18n/routing";
import { hasLocale } from "next-intl";

/**
 * OAuth / magic-link redirect target.
 *
 * Exchanges the `code` for a session, then gates first-time users (no recorded
 * legal consent) through /consent before letting them into the app.
 *
 * App routes live under `/{locale}/`, but this callback is outside `[locale]`.
 * The active locale is carried via the `locale` query param (set when starting
 * OAuth) so every redirect destination is locale-prefixed; falls back to the
 * default locale if absent/invalid.
 */
export async function GET(request: Request) {
  const { searchParams, origin } = new URL(request.url);
  const code = searchParams.get("code");
  const next = sanitizeNext(searchParams.get("next"));
  const consentGiven = searchParams.get("consent") === "1";

  const rawLocale = searchParams.get("locale");
  const locale = hasLocale(routing.locales, rawLocale)
    ? rawLocale
    : routing.defaultLocale;
  // Prefix a same-origin path with the active locale: "/guide" → "/vi/guide".
  const localized = (path: string) => `${origin}/${locale}${path}`;

  if (!code) {
    return NextResponse.redirect(localized("/login?error=oauth"));
  }

  const supabase = createClient();
  const { error } = await supabase.auth.exchangeCodeForSession(code);
  if (error) {
    return NextResponse.redirect(localized("/login?error=oauth"));
  }

  const {
    data: { user },
  } = await supabase.auth.getUser();

  // First-time sign-ups (consent recorded just now) land on the guide page;
  // returning users go to their requested destination.
  let firstTime = false;

  if (user) {
    // The profile row is created by the on_auth_user_created DB trigger.
    const { data: profile } = await supabase
      .from("users")
      .select("terms_agreed_at")
      .eq("id", user.id)
      .maybeSingle();

    if (!profile?.terms_agreed_at) {
      // Consent already given on the sign-up page → record it and continue.
      if (consentGiven) {
        const now = new Date().toISOString();
        await supabase
          .from("users")
          .update({
            terms_agreed_at: now,
            terms_version: TERMS_OF_SERVICE.lastUpdated,
            privacy_agreed_at: now,
            privacy_version: PRIVACY_POLICY.lastUpdated,
          })
          .eq("id", user.id);
        firstTime = true;
      } else {
        // Otherwise (e.g. arrived via the login page) gate through /consent (A3).
        const consent = new URL(localized("/consent"));
        consent.searchParams.set("next", next);
        return NextResponse.redirect(consent.toString());
      }
    }
  }

  const destination = firstTime ? "/guide" : next;
  return NextResponse.redirect(localized(destination));
}

/** Only allow same-origin relative paths to prevent open-redirect. */
function sanitizeNext(value: string | null): string {
  if (value && value.startsWith("/") && !value.startsWith("//")) {
    return value;
  }
  return "/";
}
