...
export async function RealEstateHome() {
  ...
  return (
    <div className="mx-auto flex max-w-container flex-col gap-16 px-4 py-10 sm:px-6 sm:py-14">
      ...
      {/* Hero — Seoul Han River skyline background with a left-aligned text
          block; a left→right white gradient overlay keeps the copy readable
          while letting the city on the right show through (PRD §4-§5). */}
      <section className="relative overflow-hidden rounded-3xl">
        <Image
          src="/estate-hero.jpg"
          alt="Seoul Han River skyline"
          width={1280}
          height={853}
          priority
        ...
        />
        {/* Readability overlay.
            Mobile: text stacks over the image, so keep it nearly solid from the
            bottom up — only the very top (behind the sky) lets the image show.
            Desktop (sm+): a left→right white fade keeps the left text column
            bright while the city on the right shows through, plus a soft vertical
            fade so the lower copy stays legible. */}
        <div className="absolute inset-0 bg-gradient-to-t from-background via-background/92 to-background/40 sm:bg-gradient-to-r sm:from-background sm:via-background/85 sm:to-transparent" />
        <div className="absolute inset-0 hidden sm:block sm:bg-gradient-to-t sm:from-background/60 sm:to-transparent" />
        <div className="relative flex flex-col items-start gap-5 p-6 sm:max-w-lg sm:p-10 lg:p-14">
          ...
          <div className="flex flex-col items-start gap-2">
            ...
            <p className="text-xs font-medium text-foreground/70">{t("realEstate.home.hero.ctaSub")}</p>
          </div>
        </div>
      </section>
      ...
      {/* Why foreign buyers choose us — 2-column card on desktop: copy on the
          left, a consultation photo on the right (PRD §6). Stacks on mobile. */}
